...
- 创建容器：通过此前制作的前置镜像生成容器，无需重复下载前置内容。
    ...
    ```shell
    sudo docker run -itd -p 3306:3306 --name pansdu-db ubuntu:18.04 bash
    ```
...
在seafile8.0以后，ElasticSearch与seafile节点彻底分离，因此一些耗时背景任务都可以被完全外置。这些服务的节点也称为seafile-background-server节点。
...
- 创建容器
    ...
    ```shell
    sudo docker run -itd -p 8082:8082 8000:8000 8080:8080 --name pansdu-db pansdu-preq bash
    ```
