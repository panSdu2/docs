...
采取的策略是将各个组件去容器化地在本地运行，接下来你可以直接阅读“[部署步骤](#部署步骤)”。本地部署的优势就是无需重新关注系统组件问题，并且能够直接使用开发环境。
...
- 启动和停止运行
    ...
    其中容器id可以简写。例如对一串哈希`a665a45920422f9d417e4867efdc4fb8a04a1f3fff1fa07e998e86f7f7a27ae3`，可以简写为`a665a459`，也可以简写为`a`，只需保证前缀本地唯一。
...
- 退出容器会话
    ...
    `Ctrl+D`或者`exit`，退出容器会话。同时容器会停止运行。

### 部署策略

确定好节点数量以及容器分配后即可进行部署。容器间、节点间互联参考“[网络相关](#网络相关)”；部署步骤参考“[部署步骤](#部署步骤)”，但是需要按照部署策略修改一些配置。

## 部署步骤

### 数据库

对于开发部署，直接进行如下步骤；对于测试部署，选择某个节点安装数据库。
...
- 测试部署
    ...
    1. seafile-server
        ...
        其中libsearpc和libvhtp为seafile-server的通讯协议，也需要下载。

        seafile-server在本项目中的分支为`develop`，我们需要先切换至该分支。
    ...
    2. seahub
        ...
        git checkout develop
        ```
        seahub与seafile-server的通讯中用到了seafile-server中的部分内容。
...
- python依赖项版本的限定能不能该？
    ...
    不能。甚至随着一些依赖项的更新，更多的依赖项需要被限定版本。这是由于python的依赖项不一定保证向下兼容性。